...
class Webcam(object):
    ...
    def MissonApproved(self):
        ...
        for i in range(len(x)):
            x[i] = ((x[i] / self.BoundaryObject.Px2CmWidth) - 10.25 )*-1 #to far to the right removed the +1 from x axis CHANGED FROM -10.5 to 10
    ...
    def ColorRecog(self, color):
        ...
        """
        Blurring methods to get better picture quality. Not used for this iteration
            #kernel = np.ones((15,15),np.float32)/255
            #smoothed = cv2.filter2D(result, -1, kernel)
            #self.blur = cv2.GaussianBlur(result, (15,15), 0) #method of blurring to get image
        """

        #Blurring module used to hand
        self.median = cv2.medianBlur(result, 15)

        print("Original Image, masked image, and blurred image being generated")
        #display image results
        cv2.imshow("Orginal image", image)
        cv2.imshow("mask on molly percocet", self.mask)
        cv2.imshow("bluur", self.median)
